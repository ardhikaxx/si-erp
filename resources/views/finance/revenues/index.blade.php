...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="fas fa-money-bill-trend-up me-2"></i>Data Pemasukan</h5>
        <a href="{{ route('finance.revenues.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Tambah Pemasukan</a>
    </div>
    <div class="card-body">
        <div class="row mb-3">
            <div class="col-md-3">
...
$(document).ready(function() {
    $.get('{{ url("finance/chart-of-accounts") }}', {filter_type: 'revenue'}, function(data) {
        // Populate account filter from the ajax endpoint - use a dedicated data source
    });

    $.ajax({
        url: '{{ url("finance/chart-of-accounts") }}',
        data: { filter_type: 'revenue' },
        dataType: 'json',
    ...
    var table = $('#dataTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("finance.revenues.index") }}',
            data: function(d) {
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            text: 'Apakah Anda yakin ingin menghapus pemasukan "' + name + '"?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = '{{ url("finance/revenues") }}/' + id;
